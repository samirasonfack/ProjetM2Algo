...
#' graphique log-log.
#'
#' @param debut Nombre. Taille minimale des vecteurs simulés plus grand que 2 (par défaut 400).
...
#' @examples
#' \dontrun{
#' res <- simulation_dtw_rcpp(debut = 100, fin = 1000, pas = 100)
#' }
#'
#' @importFrom stats runif na.omit lm coef
#' @importFrom graphics abline
#' @export
simulation_dtw_rcpp <- function(
    debut = 400,
    fin = 5000,
    pas = 50,
    afficher_plots = TRUE
  ...
  if (afficher_plots) {
    ...
    plot(resultats$Taille, resultats$Temps, type = "b", pch = 19, col = "blue",
         main = "Complexité expérimentale de l'algorithme DTW RCPP",
    ...
    plot(log(resultats$Taille), log(resultats$Temps), type = "b", pch = 19, col = "blue",
         main = "Complexité expérimentale (log-log) de DTW RCPP",
