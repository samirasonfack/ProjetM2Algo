...
List segmental_dtw_cpp(NumericVector serie1, NumericVector serie2, int segment_length) {
  ...
  // cumulative cost pour segments suivants
  ...
  for(int j=0;j<n;j++)
    ...
    std::vector< std::pair<int,int> > path_seg_vec;
    int i = n_seg-1;
    int j = 0;
    ...
    while(i >= 0){
      path_seg_vec.push_back(std::make_pair(i,j));
      if(i==0) break;
      min_val = R_PosInf;
      int new_j = j;
      for(int k=j;k<n;k++){
    ...
    // Convertir path_seg en IntegerMatrix pour R
    IntegerMatrix path_seg(path_seg_vec.size(), 2);
    for(size_t k=0; k<path_seg_vec.size(); k++){
      path_seg(k,0) = path_seg_vec[k].first + 1; // 1-based
      path_seg(k,1) = path_seg_vec[k].second + 1;
    }
